...
#| 
library(pacman)
p_load("tidyverse", "easystats", "broom", "kableExtra", "interactions", "emmeans", "moderndive", "ggpredict")
```
...
-   Sometimes referred to as a moderator analysis
-   A moderator variable Z (water in this case) is a variable that
    alters the strength of the relationship between X and Y (sunshine
    -\> growth)
...
```{webr-r, fig.align='center', out.width="90%"}
ggplot(evals1, aes(x = age, y = score, color = gender)) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", color = "Gender") +
  geom_smooth(method = "lm", se = FALSE)
...
```{webr-r, fig.align='center', out.width="80%"}
ggplot(evals1, aes(x = age, y = score, color = gender)) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", color = "Gender") +
...
that the model significantly predicted evaluation scores accounting for
5% of the variance. Age was a significant negative predictor of scores,
*b* = -0.01, *SE* = .003, 95% CI \[-0.02, -0.01\], t = -3.89, *p* \<
.001, with increases in age being associated with lower evals. Gender
was also significant, *b* = -0.208, *SE* = .05, 95% CI \[-0.312,
-0.105\], t = -3.95, *p* \< .001, with women (M = 4.04) having lower
evals han men (M =4.24). Importantly, there was a significant
