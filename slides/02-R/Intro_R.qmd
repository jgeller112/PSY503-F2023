...
-   You can type in a document (*Script* or *Markdown*) and tell it to
...
-   The arrow `<-` is called an ASSIGNMENT OPERATOR, and tells R to save
    an object called x that has the value of 6. This is similar to
...
-   We will use the word `Variable` to refer to columns of a data frame
    and to data variables that we use in models
...
3.  Modify the function above and instead of returning the sum return
    the product.
...
-   `tidyverse` provides another interface to functionsthe pipe operator
...
-   Alter the code to select just the sex column
...
> See if you can use the logical operators to manipulate our Titanic
> data
...
With Titanic data use `%` to:

-   Filter by sex and age

    -   Females under the age of 30

-   Select columns: sex, age, and fare

-   Arrange by fare (ascending order)
