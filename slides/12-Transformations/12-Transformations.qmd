...
# data wizard from easystats
data %>%
  mutate(age_c=age-mean(age), 
         age_c_dw=datawizard::center(age))
...
-   b = unstandardized regression coefficient
    ...
    -   For every one unit increase in X, there will be b unit increase
        in Y
...
-   Logarithmic
    ...
    -   Describe changes in terms of multiplication
        ...
        -   How many times must one "base" number be multiplied by
            itself to get some other particular number?
...
lm(log(Rate)~Age, data=ex0824) %>%
  tidy(conf.int = TRUE, exponentiate = FALSE) %>% kable() %>% 
  knitr::kable() %>%
    kable_styling(font_size = 24) %>%
 row_spec(1:2, color = "white",
              background = "red")
...
-   $b_1$: For increase in age, the mean respiratory rate for a new born
    child is expected to decrease by 2% (if exp \< 1 decrease, if exp \>
    1 increase)
:::
...
-   When dependent/response variable and independent/predictor
    ...
    -   Interpret the coefficient as the percent increase in the
        ...
        -   Example: the coefficient is 0.198. For every 1% increase in
            ...
            by about 0.20%. For x percent increase, calculate 1.x to the
            power of the coefficient, subtract 1, and multiply by 100
...
-   The subjects were then asked to recall the items at various times up
    to a week later

```{r}

mem_df <- tibble::tribble(
   ~time, ~prop,
      1L,  0.84,
      5L,  0.71,
     15L,  0.61,
...
anim.1<- ggplot(mem_df, aes(time,prop))+
   geom_point(size=5)+
  theme_minimal(base_size=16)
...
log_df_quad <- mem_df%>%
  mutate(time2=time^-2) # add in quadratic term to dataset
...
quad_lm <-lm(prop ~ time + time2,  data=log_df_quad)
...
# linear model
lm_lin <- lm(prop ~ time,  data=mem_df)
...
-   If *F* is significant, less pasimonious model (more DFs) is
    preferred
