...
3.  You are going to roll some dice twice. What is the chance you roll
    double 1s?
...
-   Probablities are based on long run frequency of events

    -   Flipping a coin and getting heads is 50%

        -   You will get heads 50% of the time if coin is *flipped over
            and over again*
...
-   Random variables are translations of outcomes of a random process
    into **numbers**
...
-   Earlier, we derived the distribution of a simple rv by directly
    investigating the underlying sample space
...
-   PMF: function $p$ given by $$p(x) = P(X = x)$$ for all possible
    values of $x$
...
-   Imagine that you started a strict diet a few days ago. You are at a
    dinner partyat your friend's who made your favorite dessert. You are
    very tempted and make the decision to flip a coin three times. The
    number of times that the flip coin returns tail determines the
    number of bites of the dessert you will have. Before you start
    flipping the coin, you want to learn more about your chances
    of not having dessert tonight. To do that, you decide to look at the
    possible outcomes. You let 𝑋 be the number of times a series of
    three coin flips returns tails (T). The support of 𝑋 is {0, 1, 2,
    3}.
...
ggplot(df, aes(x=x, y=probabilities)) +
    ...
    labs(title="PMF of number of heads in 3 coin flips",
         x="Number of heads",
         y="Probability") +
    theme_minimal() +
...
-   The PDF of a random variable is the function $F$ such that
    $F(x) = P(a \geq X \geq b)$
...
-   PMFs and CDFs are very useful tools to summarize information from
    rvs.
...
::: {.column width="50%"}
![](https://psy503-found-stats.netlify.app/slides/05-probability/basics_of_probability_theory_files/figure-html/unnamed-chunk-9-1.png){fig-align="center"
width="649" height="364"}
:::
:::
...
Sampling helps us build up our intuitions about probablity
distribiutions
...
::: columns
::: {.column width="50%"}
![](/slides/02-R/images/ggplot.PNG){fig-align="center"}
:::
